...
bool isGpuAvailable() {
    ...
    for(int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
        ...
        if (deviceProperties.major >= 2 && deviceProperties.minor >= 0) {
            ...
            printf("selected device %d", deviceIndex);
            return true;
        }
    }
    return false;
...
// -----------------------------------------------------------------------------------------------
   ...
   for ( i=38; i<42;i++){
    ...
    for ( myTime=1; myTime<tmax; myTime++){  // This for loop takes care of myTime evolution
        ...
        before_poisson_cu<<<512, N/512>>>(imax, jmax, kmax, tmax, ne, ni, difxne, difyne, difxni, difyni, difxyne, difxyni, Exy, fexy, fixy, g, g_temp, R, Ex, Ey, fex, fey, fix, fiy, V, L, difzne, difzni, Ez, fez, fiz, qi, qe, kr, ki, si, sf, alpha, q, pie,Ta ,w ,eps0 , Te, Ti, B, Kb, me, mi, nue, nui, denominator_e, denominator_i, nn, dt, h, wce, wci, mue, mui, dife, difi);
        ...
        after_poisson_cu<<<512, N/512>>>(imax, jmax, kmax, tmax, ne, ni, ne_temp, ni_temp, difxne, difyne, difxni, difyni, difxyne, difxyni, Exy, fexy, fixy, g, g_temp, R, Ex, Ey, fex, fey, fix, fiy, V, L, difzne, difzni, Ez, fez, fiz, qi, qe, kr, ki, si, sf, alpha, q, pie,Ta ,w ,eps0 , Te, Ti, B, Kb, me, mi, nue, nui, denominator_e, denominator_i, nn, dt, h, wce, wci, mue, mui, dife, difi, sf_temp);
