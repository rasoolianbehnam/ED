...
__global__ void after_poisson_cu(int imax, int jmax, int kmax, int tmax, float *ne, float *ni
                                , float *ne_temp, float* ni_temp, float *difxne, float *difyne, float *difxni
                                , float *difyni, float *difxyne, float *difxyni, float *Exy, float *fexy
                                , float *fixy, float *g, float* g_temp, float *R, float *Ex, float *Ey
                                , float *fex, float *fey, float *fix, float *fiy, float *V, float *L
                                , float *difzne, float *difzni, float *Ez, float *fez, float *fiz
                                , float *values, float *sf_temp) {
    int index_x = threadIdx.x + blockDim.x * blockIdx.x;
    int stride_x = blockDim.x * gridDim.x;
    int  n1=imax+3, n2 = jmax+3, n3 = kmax+3,i,j,k,myTime,kk,I,N,s1;
    N=n1*n2*n3;
...
// -----------------------------------------------------------------------------------------------
   ...
   for ( i=38; i<42;i++){
    ...
    for ( myTime=1; myTime<tmax; myTime++){  // This for loop takes care of myTime evolution
        ...
        before_poisson_cu<<<512, N/512>>>(imax, jmax, kmax, tmax, ne, ni, difxne, difyne, difxni, difyni, difxyne, difxyni, Exy, fexy, fixy, g, g_temp, R, Ex, Ey, fex, fey, fix, fiy, V, L, difzne, difzni, Ez, fez, fiz, values);



        for (kk=0; kk<iterations; kk++) {
            poisson_solve_1it_cu<<<512, N/512>>>(imax, jmax, kmax, n1, n2, n3, N, iterations, V, g, R, w, h, 1);
            poisson_solve_1it_cu<<<512, N/512>>>(imax, jmax, kmax, n1, n2, n3, N, iterations, V, g, R, w, h, 0);
        }



        after_poisson_cu<<<512, N/512>>>(imax, jmax, kmax, tmax, ne, ni, ne_temp, ni_temp, difxne, difyne, difxni, difyni, difxyne, difxyni, Exy, fexy, fixy, g, g_temp, R, Ex, Ey, fex, fey, fix, fiy, V, L, difzne, difzni, Ez, fez, fiz, values, sf_temp);
