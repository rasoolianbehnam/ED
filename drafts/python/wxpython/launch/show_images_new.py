...
if "linux" in system:
    pip_path = "%s/.local/bin/pip"%home
elif "darwin" in system:
    pip_path = "%s/Library/Python/2.7/bin/pip"%home
if not os.path.exists(pip_path):
    os.system("curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py && python get-pip.py --user")
try:
    import wx
    print("wx is installed")
except:
    os.system('%s install --user wxpython'%pip_path)

try:
    import cv2
    print("cv2 is installed")
except:
    os.system('%s install --user opencv-python'%pip_path)
...
class ShowCapture(wx.Panel):
    ...
    def NextFrame(self, event):
        ...
        if time.time() - self.start > episode_length:
            ...
            if self.mode == 'test':
                print(time.time()-self.start)
                if time.time() - self.start > episode_length / 2:
                    print("mardas")
                    self.doprintAnswers()
